...
namespace Application
{
	 ...
	 *
	 */
    LogTextCtrl::LogTextCtrl(Window* aParent, WindowId aWindowId /*  = DEFAULT_ID */,
                             long aStyle /* = wxTE_READONLY | wxTE_MULTILINE | wxTE_DONTWRAP */,
                             const std::string& anInitialText /* = "" */, const Point& aPoint /* = DefaultPosition */,
                             const Size& aSize /* = DefaultSize */)
        : wxTextCtrl(aParent, aWindowId, WXSTRING(anInitialText), aPoint, aSize, aStyle)
    {
        Bind(ROBOTWORLD_TRACE_EVENT, [this](CommandEvent& anEvent) {
            this->OnCommandEvent(anEvent);
        });
    }
    /**
	 *
