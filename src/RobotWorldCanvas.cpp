...
namespace View
{
    ...
    RobotWorldCanvas::RobotWorldCanvas(Window* anOwner)
        : ScrolledCanvas(anOwner, DEFAULT_ID), popupPoint(DefaultPosition), startActionPoint(DefaultPosition),
          endActionPoint(DefaultPosition), actionOffset(DefaultPosition), startActionSize(DefaultSize),
          startActionShape(nullptr), endActionShape(nullptr), selectedShape(nullptr), activationEnabled(false),
          selectionEnabled(false), menuItemEnabled(false), dandEnabled(true), notificationHandler(nullptr)
    {
	 ...
	 *
	 */
    RobotWorldCanvas::RobotWorldCanvas(Window* anOwner, Model::ModelObjectPtr aModelObject)
        :

          ScrolledCanvas(anOwner, DEFAULT_ID), ViewObject(aModelObject), popupPoint(DefaultPosition),
          startActionPoint(DefaultPosition), endActionPoint(DefaultPosition), actionOffset(DefaultPosition),
          startActionSize(DefaultSize), startActionShape(nullptr), endActionShape(nullptr), selectedShape(nullptr),
          activationEnabled(false), selectionEnabled(false), menuItemEnabled(false), dandEnabled(true),
    ...
    void RobotWorldCanvas::setSelectedShape(ShapePtr aSelectedShape)
    {
        ...
        if (previousSelectedShape && aSelectedShape &&
            previousSelectedShape->getObjectId() != aSelectedShape->getObjectId())
        {
    ...
    {
        return std::any_of(shapes.begin(), shapes.end(), [&aPoint](ShapePtr aShape) {
    ...
    {
        if (auto i = std::find_if(shapes.begin(),
                                  shapes.end(),
                                  [&aPoint](ShapePtr shape) {
                                      return shape->occupies(aPoint);
                                  });
            i != shapes.end())
        {
            return *i;
    ...
    void RobotWorldCanvas::initialise()
    {
        ...
        notificationHandler =
                new Base::NotificationHandler<std::function<void(NotifyEvent&)>>([this](NotifyEvent& anEvent) {
                    this->OnNotificationEvent(anEvent);
                });
    ...
    void RobotWorldCanvas::render(wxDC& dc)
    {
        ...
        if (startActionShape != nullptr && actionStatus == DRAWING)
        {
            ...
            dc.DrawLine(startActionShape->getCentre().x,
                        startActionShape->getCentre().y,
                        endActionPoint.x,
                        endActionPoint.y);
        }
    }
    ...
    {
        WayPointShapePtr wayPoint(
                new WayPointShape(Model::RobotWorld::getRobotWorld().newWayPoint("Joost", popupPoint)));
        addShape(wayPoint);
        Refresh();
    }
    /**
    ...
    void RobotWorldCanvas::handleAddWall(CommandEvent& UNUSEDPARAM(event))
    {
        ...
        ShapePtr wall(
                new WallShape(Model::RobotWorld::getRobotWorld().newWall(start->getCentre(), end->getCentre(), false),
                              start,
                              end));
	 ...
	 *
	 */
    void RobotWorldCanvas::handleItemMenu(ShapePtr aSelectedShape, const Point& UNUSEDPARAM(aPoint))
    {
        Menu popupMenu;
