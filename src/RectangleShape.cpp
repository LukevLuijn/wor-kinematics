...
namespace View
{
    ...
    RectangleShape::RectangleShape(const std::string& aTitle /*= ""*/)
        : centre(-1, -1), size(0, 0), title(aTitle), titleSize(0, 0), borderWidth(2), spacing(2)
    {
    }
    /**
	 *
	 */
    RectangleShape::RectangleShape(Model::ModelObjectPtr aModelObject, const Point& aCentrePoint,
                                   const std::string& aTitle /*= ""*/, int aBorderWidth /*= 2*/, int aSpacing /*= 2*/)
        : Shape(aModelObject), centre(aCentrePoint), size(0, 0), title(aTitle), titleSize(0, 0),
          borderWidth(aBorderWidth), spacing(aSpacing)
    {
    }
    /**
	 *
	 */
    RectangleShape::RectangleShape(const Point& aCentrePoint, const std::string& aTitle /*= ""*/,
                                   int aBorderWidth /*= 2*/, int aSpacing /*= 2*/)
        : centre(aCentrePoint), size(0, 0), title(aTitle), titleSize(0, 0), borderWidth(aBorderWidth), spacing(aSpacing)
    {
    }
    /**
	 *
    ...
    {
        if (centre.x - (size.x / 2) <= aPoint.x && aPoint.x <= centre.x + (size.x / 2) &&
            centre.y - (size.y / 2) <= aPoint.y && aPoint.y <= centre.y + (size.y / 2))
        {
            return true;
        }
        return false;
    ...
    bool RectangleShape::isBorderPoint(const Point aPoint, int aRadius /*= 3*/) const
    {
