...
namespace Messaging
{
    ...
    public:
        /**
			 *
			 */
        explicit Session() : socket(CommunicationService::getCommunicationService().getIOContext())
        {
        }
        /**
			 *
        ...
        explicit Session(const Message& aMessage)
            : socket(CommunicationService::getCommunicationService().getIOContext()), message(aMessage)
        {
        }
        /**
			 *
    ...
    protected:
        ...
        /**
			 ...
			 */
        void handleHeaderRead(const boost::system::error_code& error, size_t UNUSEDPARAM(bytes_transferred))
        {
            if (!error)
            {
                message.setHeader(Message::MessageHeader(
                        std::string(headerBuffer.begin(), headerBuffer.end())));// @suppress("Symbol is not resolved")
        ...
        /**
			 ...
			 */
        void handleBodyRead(const boost::system::error_code& error, size_t bytes_transferred)
        {
            if (!error)
            {
                message.setBody(
                        std::string(bodyBuffer.begin(), bodyBuffer.end()));// @suppress("Symbol is not resolved")
        ...
        /**
			 ...
			 */
        void handleMessageRead(const boost::system::error_code& error, size_t UNUSEDPARAM(bytes_transferred))
        {
            if (!error)
            {
                handleMessageRead();
        ...
        void writeMessage(const Message& aMessage)
        {
            ...
            boost::asio::async_write(
                    socket,// @suppress("Invalid arguments")
                    boost::asio::buffer(message.getHeader().toString(), message.getHeader().getHeaderLength()),
                    [this, aMessage](const boost::system::error_code& error,
                                     std::size_t UNUSEDPARAM(bytes_transferred)) {
    ...
    public:
        /**
			 ...
			 */
        explicit ServerSession(RequestHandlerPtr aRequestHandler) : requestHandler(aRequestHandler)
        {
    ...
    public:
        /**
			 ...
			 */
        ClientSession(const Message& aMessage, ResponseHandlerPtr aResponseHandler)
            : Session(aMessage), responseHandler(aResponseHandler)
        {
            sessionNumber = ++sessionCounter;
        }
        /**
