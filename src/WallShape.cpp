...
namespace View
{
    ...
    WallShape::WallShape(Model::WallPtr aWall)
        : LineShape(std::dynamic_pointer_cast<Model::ModelObject>(aWall),
                    ...
                    RectangleShapePtr(new RectangleShape(aWall->getPoint2())), "", 1, 0)
    {
    }
    /**
	 *
	 */
    WallShape::WallShape(Model::WallPtr aWall, RectangleShapePtr aRectangleShape1, RectangleShapePtr aRectangleShape2)
        : LineShape(std::dynamic_pointer_cast<Model::ModelObject>(aWall), aRectangleShape1, aRectangleShape2, "", 1, 0)
    {
    }
    /**
	 *
	 ...
	 */
    // cppcheck-suppress unusedFunction
    void WallShape::setSelectedAt(const Point& aPoint /*= DefaultPosition*/, bool aSelected /*= true*/)
    {
        UNUSEDCAST(aPoint);
    ...
    {
        return getNode1()->getObjectId() == aRectangleShape->getObjectId() ||
               getNode2()->getObjectId() == aRectangleShape->getObjectId();
    }
    ...
    void WallShape::draw(wxDC& dc)
    {
        ...
        dc.DrawLine(getNode1()->getCentre().x,
                    getNode1()->getCentre().y,
                    getNode2()->getCentre().x,
                    getNode2()->getCentre().y);
    }
    /**
	 *
	 */
    ...
    {
        if (getNode1()->occupies(aPoint) || getNode2()->occupies(aPoint) ||
            Utils::Shape2DUtils::isOnLine(getBegin(), getEnd(), aPoint, 2))
        {
            return true;
        }
        return false;
    ...
    std::string WallShape::asString() const
    {
        ...
        os << LineShape::asDebugString() << " WallShape: " << Utils::Shape2DUtils::asString(getBegin()) << "->"
