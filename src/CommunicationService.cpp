...
namespace Messaging
{
	 ...
	 *
	 */
    void CommunicationService::registerServer(ServerPtr aServer, bool start /* = true */)
    {
	 ...
	 *
	 */
    void CommunicationService::stopServer(ServerPtr aServer, bool deregister /* = true */)
    {
	 ...
	 *
	 */
    void CommunicationService::stopServer(unsigned short aPort, bool deregister /* = true */)
    {
        // TODO See above
        auto result = servers.find(aPort);
        if (result != servers.end())
        {
            ...
            if (servers.empty())
            {
                ...
                timer.async_wait(
                        [this](const boost::system::error_code& UNUSEDPARAM(e))// @suppress("Method cannot be resolved")
                        {
    ...
    {
        std::thread new_io_contextThread([this] {
            run_io_context();
        });
